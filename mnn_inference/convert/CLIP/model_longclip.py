...
# import sys
# sys.path.append("/home/liu/fcb1/reft/CLIP")
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention_v2(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)
    
    def forward_v2(self, x: torch.Tensor):
        output, attn_mask = self.attention_v2(self.ln_1(x))
        x = x + output
        x = x + self.mlp(self.ln_2(x))
        return x, attn_mask
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text): 
        ...
        # x = x + 0.001 * torch.randn_like(x, dtype=x.dtype, device=x.device)
    ...
    def generate_random_feature(self, size, device):
        # self.mean, self.std = torch.tensor([0.]).to(device), torch.tensor([1.]).to(device)
        self.random_dist = dists.Normal(loc=torch.tensor([0.]), scale=torch.tensor([1.]))
        f_rand = self.random_dist.sample(size).to(device)
    ...
    def forward(self, image, text):
        image_features = self.encode_image(image)
        text_features = self.encode_text(text)
        # image_pattern_feat  = self.PCA(image_features, 32)
        # text_fine_grained = self.PCA(text_features, 32)
        # text_pattern_feat = self.encode_text(text_short)


        # normalized features
        image_features = image_features / image_features.norm(dim=1, keepdim=True)
        text_features = text_features / text_features.norm(dim=1, keepdim=True)
        # image_pattern_feat  = image_pattern_feat  / image_pattern_feat.norm(dim=1, keepdim=True)
        # text_pattern_feat  = text_pattern_feat  / text_pattern_feat.norm(dim=1, keepdim=True)
        
        sim = torch.matmul(image_features, text_features.T)
        sim = self.logit_scale.exp() * sim
        
        # sim_f = torch.matmul(image_pattern_feat,text_pattern_feat.T)
        # sim_f = self.logit_scale.exp() * sim_f
        # sim_f = torch.sum(image_fine_grained * text_fine_grained, dim=-1)
        

        bs = image.size(0)
        targets = torch.range(0, bs-1, dtype=torch.long).to(image.device)
        
        # loss_con = (
        #         F.cross_entropy(sim, targets, label_smoothing=0.1)
        #         + F.cross_entropy(sim.t(), targets, label_smoothing=0.1)
        #     ) / 2.
        ...
        # text_sim = torch.matmul(text_pattern_feat, text_pattern_feat.T)
        # text_sim = text_sim.detach()
        # targets = text_sim*0.2 + 6*torch.eye(bs, dtype=image_features.dtype, device=image_features.device)
        # targets = torch.softmax(targets, dim=-1)
 
        # loss_fine = (
        #         F.cross_entropy(sim_f, targets)
        #         + F.cross_entropy(sim_f.t(), targets.t())
        #     ) / 2.
        
        share_ref_feat = self.generate_random_feature(image_features.size(), image_features.device)
        share_ref_feat = share_ref_feat.type(self.dtype)
        loss_fine = F.mse_loss(image_features, share_ref_feat) + F.mse_loss(text_features, share_ref_feat)
        
        return loss_con, loss_fine,image_features,text_features
